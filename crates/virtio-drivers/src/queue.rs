...
pub struct VirtQueue<H: Hal, const SIZE: usize> {
    ...
    event_idx: bool,
    /// Whether used-buffer notifications are currently suppressed.
    suppressed: bool,
    #[cfg(feature = "alloc")]
    indirect: bool,
    #[cfg(feature = "alloc")]
...
impl<H: Hal, const SIZE: usize> VirtQueue<H, SIZE> {
    ...
    ///
    /// With `VIRTIO_F_EVENT_IDX` this has no effect; use
    /// [`suppress_dev_notify`](Self::suppress_dev_notify) /
    /// [`arm_dev_notify_and_check`](Self::arm_dev_notify_and_check) instead.
    ...
    /// Suppresses used buffer notifications for this queue.
    ///
    /// With `VIRTIO_F_EVENT_IDX`, `used_event` is set to
    /// `last_used_idx - 1` so the device does not notify on the next
    /// completion; without it, the `VRING_AVAIL_F_NO_INTERRUPT` flag is set.
    /// See Virtio v1.1 2.6.7 Used Buffer Notification Suppression.
    pub fn suppress_dev_notify(&mut self) {
        if self.event_idx {
            // Safe because self.avail points to a valid, aligned, initialised, dereferenceable,
    ...
    /// Rearms used buffer notifications and reports whether a completion is
    /// still pending after the memory barrier.
    ///
    /// With `VIRTIO_F_EVENT_IDX`, `used_event` is set to the current
    /// `last_used_idx`; without it, `VRING_AVAIL_F_NO_INTERRUPT` is cleared.
    /// A strong fence orders the rearm against the device's used-index write,
    /// then the used ring is rechecked. Returns `true` if a completion is
    /// pending.
    pub fn arm_dev_notify_and_check(&mut self) -> bool {
        if self.event_idx {
            // Safe because self.avail points to a valid, aligned, initialised, dereferenceable,
            // readable instance of AvailRing.
            unsafe {
        ...
        // The device may have written a used element concurrently; a strong
        // fence orders our `used_event`/flag write before the recheck so no
    ...
    pub unsafe fn pop_used<'a>(
        ...
        if self.event_idx && !self.suppressed {
            unsafe {
                (*self.avail.as_ptr())
                    .used_event
                    .store(self.last_used_idx, Ordering::Release);
...
mod tests {
    ...
    #[test]
    fn suppress_event_idx_writes_used_event() {
        let mut config_space = ();
        let state = Arc::new(Mutex::new(State {
            queues: vec![QueueStatus::default()],
            ..Default::default()
        ...
        // used_event is last_used_idx - 1, so the device does not notify on
        // the next completion.
        assert_eq!(
            unsafe { (*queue.avail.as_ptr()).used_event.load(Ordering::Acquire) },
            0u16.wrapping_sub(1)
        );
        // The driver-side flag is not used in event-idx mode.
        assert_eq!(
            unsafe { (*queue.avail.as_ptr()).flags.load(Ordering::Acquire) },
            0x0
        );
    ...
    /// While suppressed, popping used elements must not re-arm `used_event`.
    #[test]
    fn suppressed_pop_does_not_rearm() {
        let mut config_space = ();
        let state = Arc::new(Mutex::new(State {
            queues: vec![QueueStatus::default()],
            ..Default::default()
        ...
        // Two buffers: one device-readable, one device-writable (receive style).
        let token0 = unsafe { queue.add(&[], &mut [&mut [0; 4]]) }.unwrap();
        let token1 = unsafe { queue.add(&[], &mut [&mut [0; 4]]) }.unwrap();
        assert_eq!(token0, 0);
        assert_eq!(token1, 1);
        ...
        // to the new last_used_idx.
        assert_eq!(
            unsafe { (*queue.avail.as_ptr()).used_event.load(Ordering::Acquire) },
            0u16.wrapping_sub(1)
        );

        // Arming now reports pending completion (the second buffer).
    ...
    /// Arming an empty queue returns no pending completion.
    #[test]
    fn arm_empty_queue_reports_no_pending() {
        let mut config_space = ();
        let state = Arc::new(Mutex::new(State {
            queues: vec![QueueStatus::default()],
            ..Default::default()
    ...
    /// Arming after the device completed a buffer reports pending completion.
    #[test]
    fn arm_with_pending_completion_reports_pending() {
        let mut config_space = ();
        let state = Arc::new(Mutex::new(State {
            queues: vec![QueueStatus::default()],
            ..Default::default()
    ...
    /// Non-event-idx suppression uses the avail ring flag and still rechecks.
    #[test]
    fn suppress_flags_without_event_idx() {
        let mut config_space = ();
        let state = Arc::new(Mutex::new(State {
            queues: vec![QueueStatus::default()],
            ..Default::default()
    ...
    /// `used_event` and `last_used_idx` wrap around at u16 boundaries.
    #[test]
    fn event_idx_u16_wrap() {
        let mut config_space = ();
        let state = Arc::new(Mutex::new(State {
            queues: vec![QueueStatus::default()],
            ..Default::default()
        ...
        // Suppression of the next (wrapping) index writes u16::MAX - 1.
        queue.suppress_dev_notify();
        assert_eq!(
            unsafe { (*queue.avail.as_ptr()).used_event.load(Ordering::Acquire) },
            u16::MAX.wrapping_sub(1)
        );

        // Arming writes the wrapping current index and rechecks.
        let pending = queue.arm_dev_notify_and_check();
        assert!(!pending);
        assert_eq!(
            unsafe { (*queue.avail.as_ptr()).used_event.load(Ordering::Acquire) },
