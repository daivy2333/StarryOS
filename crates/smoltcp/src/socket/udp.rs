...
impl<'a> Socket<'a> {
    ...
    /// Check whether the transmit buffer holds an undispatched packet.
    ///
    /// This is a read-only observation of TX occupancy: it is true when at
    /// least one datagram is still queued and false once `dispatch` dequeued
    /// it. Unlike [`can_send`](#method.can_send), it is not affected by the
    /// buffer filling up; it reports actual pending work, not free capacity.
    #[inline]
...
mod test {
    ...
    fn test_has_pending_tx_transitions(#[case] medium: Medium) {
        let (mut iface, ..) = setup(medium);
        let cx = iface.context();
        let mut socket = socket(buffer(0), buffer(1));

        assert!(
            !socket.has_pending_tx(),
            "empty TX buffer must report no pending TX"
        );
        ...
        assert!(
            socket.has_pending_tx(),
            "one queued datagram must report pending TX"
        );
        ...
        assert!(
            !socket.has_pending_tx(),
            "successful dispatch must drain the TX buffer"
        );
    }
