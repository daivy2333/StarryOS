...
use crate::Uart16550;
use crate::{
    backend::RegisterAddress,
    spec::{FIFO_SIZE, NonIntegerDivisorError},
};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidStride(stride) => {
                write!(
                    f,
                    "invalid stride {stride}: must be non-zero and a power of two (typically 1, \
                     2, 4, or 8)"
                )
            }
        }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnexpectedLoopbackByte { expected, actual } => {
                write!(
                    f,
                    "loopback test failed: read unexpected byte! expected={expected}, \
                     actual={actual}"
                )
            }
            Self::UnexpectedLoopbackMsg { expected, actual } => {
                ...
                write!(
                    f,
                    "loopback test failed: read unexpected string! expected (str)={expected:?}, \
                     actual (str)={maybe_actual_str:?}, actual (bytes)={actual:?}"
                )
            }
        }
    }
