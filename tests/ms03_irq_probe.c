...
/* Fixed MS03 V1 ABI. The lengthless ioctl writes exactly 8 * 8 bytes. */
struct irq_snapshot {
    uint64_t total;
    uint64_t used_ring;
...
_Static_assert(sizeof(struct irq_snapshot) == 8 * sizeof(uint64_t),
               "MS03 snapshot V1 must remain 64 bytes");
