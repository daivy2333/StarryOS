...
#define BENCH_VERSION "q32-console-cpu-efficiency-20260722"
...
static void print_manifest(void) {
    ...
    printf("  bench_version_extra=q32-console-cpu-efficiency\r\n");
    printf("\r\n");
}
...
static void test_tx_enqueue_no_drain(void) {
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        printf("  write_semantics=synchronous-blocking completion=final-tcdrain-after-loop policy=no-drain size=%d iters=%d bytes=%zu short_writes=%d enqueue_ms=%ld final_drain_ms=%ld final_drain_rc=%d final_drain_errno=%d drain_calls=%d drain_errors=%d last_drain_errno=%d line_time_ms=%.1f enqueue_kbps=%.2f\r\n",
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        for (int round = 0; round < S41_ROUNDS; round++) {
            ...
            if (completion_ok && instret_ok) {
                ...
                printf("  diag=s41-invalid size=%d round=%d completed=%zu expected=%zu completion_ok=%d byte_ok=%d incomplete_logical=%d drain_ok=%d drain_rc=%d drain_errors=%d instret_begin_status=%d instret_end_status=%d instret_ok=%d begin_reason=%s end_reason=%s instret_begin=%llu instret_end=%llu\r\n",
                       test_size, round + 1, total_completed, expected_bytes,
                       completion_ok, byte_ok, incomplete_logical,
...
static void test_tx_compute_overlap(void) {
    ...
    for (int round = 0; round < S42_WARMUP_ROUNDS + S42_SAMPLE_ROUNDS; round++) {
        ...
        double overlap_efficiency = classify_overlap_efficiency(uart_iters, idle_iters);
    ...
    if (fd >= 0) {
        ...
        if (load_buf) {
            ...
            for (int g = 0; g < S43_GROUPS; g++) {
                ...
                int overlap_ok = classify_s43_loaded_applicable(write_dur_ns,
