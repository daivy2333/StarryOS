...
> 日期：2026-07-22
> 范围：QEMU 与 D1、async UART 与 polling Console 的 2×2 交叉对比，含 S41-S43 CPU 效率
...
CPU 效率差异显著。D1 Console S41 的 `instructions/byte` 在 64/256/1024B 分别为 1194、1105、1106；async UART 为 32818、32792、44716。Console 的同步写路径不需要 copier 任务、ring buffer 操作和中断驱动唤醒，instret 开销约为 Async 的 1/30。两组数据都是 hart-wide CPU-work proxy，不是 CPU utilization。
...
S42 展示了两条路径的根本差异。D1 Console 同步写把整个发送过程纳入 `write()` 调用窗口（554.66 ms），write 返回前 UART 已发送完毕 → 通信与计算之间没有剩余 overlap（overlap_efficiency=0.0000）。D1 async 的 `write()` 只在 ring 入队后立即返回（1.6 ms），UART 在后台继续发送，留下约 53.5% 的 overlap 窗口用于计算。

S43 timer wakeup overshoot 在 D1 两组相近：Console idle P50=8.42 ms，Async idle P50=9.53 ms。Console 的 loaded 组为 `not-applicable`——同步写耗尽发送窗口后没有剩余时间观测 loaded 唤醒。Async 的 loaded P50=25.78 ms，代表发送积压期间的唤醒干扰。

两种后端的所有 TX section 都完成到 `Done.`，`drain_errors=0`。所有 S43 aggregate 标记 `not-independently-recomputed`（每组仅输出 3/50 raw samples）。`instret` 为 hart-wide proxy，含同 hart 背景活动。Console S40 telemetry 为 `UNSUPPORTED`。当前数据不声明 SMP 正确性。
...
Async 和 Console 使用相同的 sizes、100 次迭代和 drain policy。QEMU 使用 ext4 rootfs；D1 使用 memory-root command-entry。设备均为 `/dev/console`。Async benchmark version 为 `q31-cpu-efficiency-20260721`，Console 为 `q32-console-cpu-efficiency-20260722`；benchmark source code 不同（Async 不含 Console 适配字段），但 S41/S42/S43 的 payload、轮数、完成点和 raw 字段完全一致，由 Q31 iteration 003 Plan Review 和 Q32 iteration 001/002 交叉验证。

Console 日志已修正标题为 `Console Benchmark`。S00 manifest 含 `backend=polling-console`。Async 日志没有 `backend` 字段，由冻结基线和启动日志识别。D1 Console S43 hang 问题（根因 IRQ stub）已通过 feature 修复并在 Q32 iteration 001/002 中验证；采集数据来自修复后的内核。

QEMU UART 模型不仿真物理线延迟，`line_rate_pct` 大于 100% 是预期现象；QEMU 只作为路径、接口和相对行为证据。D1 数据来自物理 UART0，是绝对线速依据。

统计口径：P50/P95/P99 都来自每组 N 次样本排序后的分位值。P50 代表典型耗时，P99 用来观察少数慢样本；`P99/P50` 越高，说明尾部抖动越明显。`line+10ms` tail 指单次耗时超过该 payload 的理论线时再加 10 ms。
...
S11 在两种后端上的语义不同。async 计时窗口测 ring enqueue，随后 final drain；Console 的 write loop 已同步发送，不能把 `enqueue_kbps` 当作入队速度。Console 输出明确标注 `write_semantics=synchronous-blocking completion=final-tcdrain-after-loop`。表格并列行为，不计算 Console/async 倍率。
...
S41 测 TX CPU Work。测试代码在 `instret` 区间内完成 100 次 `write_full()` + `tcdrain()` 完整发送链，分 64B、256B、1024B 三种 payload，各做 5 轮。输出 raw instret delta、instructions/byte、instructions/write-call 和中位数 summary。`instret` 是 hart-wide CPU-work proxy，不是 CPU utilization。

| Payload | QEMU async inst/byte median | QEMU Console inst/byte median | D1 async inst/byte median | D1 Console inst/byte median |
|---|---|---|---:|---:|---:|
| 64B | 14358 | 14779 | 32818 | 1194 |
| 256B | 13504 | 14173 | 32792 | 1105 |
| 1024B | 13843 | 13757 | 44716 | 1106 |

D1 Console 三条 payload 的 `instructions/byte` 稳定在 1100 左右，同步 `write() → polling send → tcdrain()` 路径不需要 copier 任务和中断唤醒。D1 async 的 64B 和 256B 约 32800 inst/byte；1024B 升至 44716 inst/byte，受 ring backpressure 和 `tcdrain` 卡在 TEMT 的影响。Console 的 instret 开销约为 Async 的 1/30（64/256B）到 1/40（1024B）。

QEMU 的 instret 在 13500-14800 之间，两组差距小（虚拟 UART 无物理线速限制，qemu-system 自身开销主导）。QEMU 的 instret 不作为硬件性能证据。

所有 15 轮均为 valid，`short_writes=0`、`drain_errors=0`、`incomplete_logical=0`。Console 的 S41-local-counters 为 `not-available`（无 async TX 计数器）。

S42 测 TX Compute Overlap。测试代码先测量纯计算 idle 基线（固定时间内能完成多少次 compute kernel），再在 64B×100 的 UART write 窗口内做同样的 compute kernel，比较 UART 发送期间可执行的有效计算量。
...
D1 Console 的 `write_return_us ≈ 554.66 ms`，已经超过 64B×100 的理论线时 542.535 ms。同步写把整个 UART 发送过程纳入调用窗口，`write()` 返回前数据已发送完毕，因此与 idle 基线之间没有剩余 overlap（efficiency=0.0000）。这不是性能失败，而是同步写路径的固有语义。

D1 async 的 `write()` 在 ring 入队后约 1.6 ms 返回，UART 在后台继续发送，overlap_efficiency=0.5353。QEMU 上两组都有 overlap（虚拟 UART 极快），Console 的 QEMU overlap 接近 1.0。

S43 测 Timer Wakeup Overshoot。测试代码用 `clock_nanosleep(TIMER_ABSTIME)` 按 5ms 间隔做绝对时间睡眠，共 5 组 idle（纯睡眠）和 5 组 loaded（在 4096B burst write 期间睡眠）。每组 50 次采样，输出 P50/P95/P99 overshoot。
...
D1 Console idle overshoot 整体更低（P50=8.42 vs Async 9.53），且 idle P99=8.77 ms 无尾部抖动。D1 Console 的 loaded 组为 `not-applicable reason=no-overlap-window`：同步 `write()` 用 355 ms 发送 4096B，耗尽 347 ms 理论线时窗口后没有剩余时间观测 loaded 睡眠 overshoot。D1 async 的 `write()` 返回仅 0.5 ms，loaded 组 P50=25.78 ms，代表发送积压期间的唤醒干扰。

S43 每组仅输出前 3/50 raw samples，无法独立重算全部 percentile。所有 P50/P95/P99/max 值以原始日志 hash 锚定，在 comparison 中标记为 `reported, hash-anchored, not-independently-recomputed`。

## 稳定性

四组 benchmark 均执行到 `Done.`。两组 D1 日志记录进程退出码 0；S10/S12/S13/S14/S20/S21/S41/S42/S43 的 `drain_errors` 均为 0。
...
| S05 ring benchmark | available | SKIPPED | available | SKIPPED |
| TX drain errors | 0 | 0 | 0 | 0 |
| 完成状态 | `Done.` | `Done.` | `Done.`, exit 0 | `Done.`, exit 0 |

D1 Console S30 现在为 PASS/EAGAIN（与 QEMU Console 一致），不再标记 UNSUPPORTED。X30 空读行为在所有四组一致。

## CPU 与计数器代理

当前 benchmark 不采集 CPU 使用率。S40 只提供 TX counter proxy，用来判断 async 发送路径的调用形态和 fallback 是否耗尽。
...
四份日志覆盖同口径 benchmark 的 QEMU/D1 与 async/Console 组合。S41/S42/S43 为新增 CPU 效率 section，由 Q31 Async 和 Q32 Console 独立 change 实现，payload、轮数、完成点和 raw 字段经 cross-review 确认一致。可比 section 已按同平台比较；能力不一致的 section 保留 `SKIPPED`、`UNSUPPORTED`、`N/A` 或 `not-applicable`。

仍需保留的边界如下：

| 项目 | 状态 | 说明 |
|---|---|---|
| D1 Console RX | 已修复 | S30 四组均为 PASS/EAGAIN |
...
| Console S41/S42/S43 local counters | not-available | ioctl 不可用，不补零 |
| D1 Console S42 overlap=0 | 合法语义 | 同步写耗尽发送窗口，不是性能失败 |
| D1 Console S43 loaded | not-applicable | write 355ms > 347ms 理论线时，无 overlap 窗口 |
| S43 percentile | not-independently-recomputed | 每组仅 3/50 raw samples，无法独立重算 |
| `instret` | CPU-work proxy | hart-wide，不含 CPU utilization 声明 |
| CPU 使用率 | 未采集 | counter proxy 与 instret 都不能替代 CPU 百分比 |
