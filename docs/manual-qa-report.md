...
T11 / T12 测得的端到端性能数据反映**Q7 阶段**状态——单字节平均延迟 150.7 µs（其中软件开销 63.9 µs），4096B 吞吐量在真板预测下效率 97.9% 线速。**Q13 + Q13.1 + LTO 优化后**已显著改善至 129.5 µs avg / 42.6 µs 软件开销（详见 `docs/benchmark-report-async.md` §0 TL;DR）。
