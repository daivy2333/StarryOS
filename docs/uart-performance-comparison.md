...
**⚠️ CPU 占用说明**：
- 当前 CPU 占用数据**不可直接对比**
- Console 测试写入 120 字节，CPU 占用 2.3%
- Async 测试写入 102,400 字节，CPU 占用 57.8%
- **数据量差 853 倍**，导致 CPU 占用差异大
- 需要统一测试数据量才能公平对比
...
- 所以两者的 write() 都很快，差异不大

**真实硬件上的预期差异**：
- **Console**：write() 需要等待 FIFO 有空位，延迟高
- **Async**：write() 只写入 Ring Buffer，延迟低
- **预期**：真实硬件上 Async 的 write() 应该快很多

**对比意义**：
- QEMU 中两者差异不大，是因为 QEMU 的限制
- 真实硬件上 Async 应该有明显优势
- 需要在 VisionFive2 上验证
...
| 指标 | Console | Async | 说明 |
|------|---------|-------|------|
| **CPU Usage** | 2.3% | 57.8% | **数据量不同** |
| **CPU Cycles** | 492,914 | 27,420,885 | **数据量不同** |
| **测试数据量** | 120 字节 | 102,400 字节 | **差 853 倍** |

**问题分析**：
- Console 测试写入 120 字节（10 行 × 12 字节）
- Async 测试写入 102,400 字节（100 次 × 1024 字节）
- **数据量差 853 倍**，导致 CPU 占用差异大
- 无法直接对比 CPU 效率

**正确的对比方式**：
1. **统一测试数据量**：两者写入相同字节数
2. **计算单位数据量的 CPU 占用**：CPU Cycles / 数据量
3. **或者测量相同操作的 CPU 占用**：如写入 1024 字节
...
| **CPU 占用** | ⚠️ 不可比 | ⚠️ 不可比 | - | 数据量不同，需统一测试 |
...
**核心结论**：
- **Async 的内核态操作快 383 倍**，因为它避免了硬件等待
- **CPU 占用数据不可直接对比**，因为测试数据量不同（差 853 倍）
- **用户态性能差异不大**，都受 QEMU 串口模拟影响
- **Console 更省内存**（0 KB vs 128 KB）
- **两者都能正常工作**，Shell 功能和数据完整性都正确

**⚠️ 重要说明**：
- CPU 占用数据**有误导性**，因为测试数据量不同
- 需要统一测试数据量才能公平对比 CPU 效率
- 当前数据不能说明 Console 更省 CPU
