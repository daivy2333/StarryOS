...
> 项目：[StarryOS](https://github.com/daivy2333/StarryOS) | 分支：asyncuart-dev | 日期：2026-06-11（Q0~Q11 完成）
...
| 缓冲区 | 无 | 128 KB（[HeapRb](https://github.com/daivy2333/StarryOS/blob/asyncuart-dev/kernel/src/drivers/ring_buffer.rs#L7) RX/TX 各 64 KB） |
| `write()` 延迟 | P50 17.5 µs（逐字节轮询 LSR） | P50 7.9 µs（push ring buf 即返回） |
...
| tcdrain | 隐式（每字节等 LSR） | ✅ [TCSBRK](https://github.com/daivy2333/StarryOS/blob/asyncuart-dev/kernel/src/syscall/fs/ctl.rs#L43) + DRAIN_WAKER 条件唤醒 |
...
Async 更快根因：`write()` 只 push 到 ring buffer（~1 µs + 锁），Console 的 `write()` 逐字节轮询 LSR 写 THR。
...
| 大小 | Async Q11 (QEMU) | 硬件理论 | 真板预测 |
|------|-----------------|----------|----------|
| 64 B | 427.1 µs | 5.56 ms | 5.98 ms |
| 256 B | 1175.0 µs | 22.2 ms | 23.4 ms |
| 1024 B | 4704.0 µs | 88.9 ms | 93.6 ms |
| 4096 B | 9052.6 µs | 355.6 ms | 364.6 ms |

---

## 5. 功能覆盖

| 功能 | Console | Async (Q11) |
|------|---------|------------|
| 阻塞读写 | ✅ | ✅ |
...
**完整优化历史**：Q0~Q4（驱动骨架）→ Q5（NAPI/批量I/O）→ Q7（yield storm/FIONBIO/tcdrain）→ Q8（NAPI退出/ISR无锁/O46 AtomicWaker）→ Q9（VTIME超时）→ Q10（BUF_SIZE 256/push_slice/&self）→ Q11（通用质量）→ Q6（真板待验证）