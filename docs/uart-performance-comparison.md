...
> **截稿日期**：2026-07-08（Q19C-M0 已落地，Q19C-M1 已通过，benchmark 数据采用同版 `q19c-m0-20260703` 横向解释）
...
| 真板吞吐量 | 📐 上限 11.52 KB/s | D1: 64B 11.13 / 256B 11.21 / 1024B 11.38 / batch-1024B 11.42 KB/s | 64B 96.6% / 256B 97.3% / 1024B 98.8% / batch-1024B 99.1% 线速（Q19C-M0 + Q19C.8e，2026-07-07 同版 `q19c-m0-20260703`）；VF2 待回填 |
...
- ❌ **不可信**：用户态吞吐（QEMU 绝对值）；D1 真板绝对值已在 Q19C-M0（2026-07-07）+ Q19C-M1（2026-07-08）回填，VisionFive2 待回填
...
|------|------------------|----------|----------|
| 64 B | 518.0 µs | 5,556 µs | D1: 11.13 KB/s / 96.6% line rate（pre-section drain 后消除 backlog 污染，Q19C-M0）|
| 256 B | 1,305.6 µs | 22,222 µs | D1: 11.21 KB/s / 97.3% line rate |
| 1024 B | 4,922.5 µs | 88,889 µs | D1: 11.38 KB/s / 98.8% line rate |
| 1024 B (batch-drain) | 4,922.5 µs | 88,889 µs | D1: 11.42 KB/s / 99.1% line rate（每 8 次 write 后 tcdrain）|

> **数据可信度**：QEMU 上的 Async 吞吐受 copier 任务切换开销影响；绝对吞吐以真板为准。Q19C-M0（2026-07-07）D1 同版 `q19c-m0-20260703` 数据：64B 96.6% / 256B 97.3% / 1024B 98.8% / batch-1024B 99.1% 115200bps 线速；Q19C-M1 已在真板通过 memory-root path loader 证明路径；4096B 长耗时项已在 Q19C-M0 manifest 移除（吞吐已稳定，无需长时采样）。VisionFive2 仍需后续单独测。**📐 物理定律**：真板 NS16550 @ 115200 bps 线速上限 = 11,520 B/s（单字节 86.8 µs），但实测值受调度/IRQ 延迟影响可能低于此值。

**小结**：用户态吞吐在 QEMU 上**不可信**（§1 标注的"❌ 不可信维度"）。Async 端到端路径已经过 Q8 优化（9→4~6 次任务切换）；Q19C-M0（2026-07-07）D1 真板结果说明大包发送已接近物理线速（96.6%~99.1%），Q19C-M1（2026-07-08）进一步在真板 memory-root path loader 上证明路径，VisionFive2 结果仍需 Q20 复验时回填。
...
1. **Async 在 7 维度全面胜出，1 维度持平**——8 维度对照见 §0，唯一持平项是真板吞吐量（受 115200 bps 物理定律约束，📐 上限 11.52 KB/s，**非实现差异**；✅ Q19C-M0 D1 真板已回填 64B 96.6% / 256B 97.3% / 1024B 98.8% / batch-1024B 99.1% 线速）
