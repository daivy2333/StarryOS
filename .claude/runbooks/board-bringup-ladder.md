...
- 新板（如 VisionFive2）的首次适配：从零到 async UART benchmark
- D1 Lichee RV Dock 已验证此阶梯有效
- 不适用于：已有平台的增量修改、QEMU 开发

## 核心原则

> 每次只开放一个变量。第一个成功的输出就是 Gate——在它之前不要引入 IRQ、async task、rootfs、benchmark。

Q19 D1 bring-up 教训：同时打开太多子系统 → 失败原因不可定位。必须按阶梯逐步推进，每层通过验证后才进入下一层。
...
- 整理硬件事实：SoC/CPU/RAM/UART base/IRQ/MMIO 宽度/boot 分区格式/启动链
...
- 编写或复用打包工具（D1：`tools/android_boot_image.py`）
- 备份官方 boot 分区并保存到开发机
...
- 实现 UART polling 写字符（不依赖 IRQ、async task、文件系统）
...
- UART IRQ claim/complete，计数打印
- **Gate**：IRQ 计数 > 0 且无丢失中断 panic

### L6: Async Console

**目标**：接回 async UART `/dev/console`。

- 在 L3-L5 稳定基础上接入 async UART driver
- 注册 PLIC handler、启动 RX/TX copier
- **Gate**：`/dev/console` 可输出、shell 或最小 I/O 可工作

### L7: Benchmark

**目标**：运行串口 benchmark 并采集数据。

- 根据启动链选择 benchmark 模式（embedded / rootfs / command-entry）
- **Gate**：benchmark exit 0；S10 line_rate_pct ≥93%；S30 双 PASS；S40 无 exh 耗尽
- 原始 log 保存到 `.claude/analysis/`
...
| L5 之前不接 async UART | async UART 依赖 PLIC IRQ，先确认 PLIC 链路再用 |
| L6 之前不接 benchmark | benchmark 依赖 `/dev/console`，先确认 console 可用 |
| 每层保持 QEMU 可构建 | board feature 和 QEMU feature 独立，改动不能破坏 QEMU 回归能力 |

## 注意事项

- **不要在 L0 阶段无限采集**：D1 已验证——现有事实覆盖 bring-up 所需后，放弃从官方 Linux 做无目标数据采集。
- **UART 访问宽度不能套 QEMU**：每块板的 UART stride 和 MMIO 宽度不同，必须按 datasheet/DTS 设置。
