...
- 将多个驱动优化 commit 从临时分支合入主干
- 不适用于：单 commit 简单修复、无副作用的格式化/typo 变更

## 为什么必须增量

核心教训：**一次性 apply 全部优化代码 → 可能造成数量级退化，且不可定位到具体 commit**。改为按依赖排序 + 每步 Gate 后，可快速完成 + 无退化。
...
cargo check --features <platform> # 编译检查
cargo clippy --features <platform># lint 检查
...
| Gate | 命令 | 通过条件 |
|---|---|---|
| 编译 | `cargo check --features <platform>` | exit 0 |
| Lint | `cargo clippy --features <platform>` | exit 0 |
| Boot | `make run` | 进入 shell |
| 1B 延迟 | benchmark latency test avg | 与基线差异 <10% |
| 64B 吞吐 | benchmark throughput test 64B | 与基线差异 <10% |
| Nonblocking | benchmark nonblocking test | 双 PASS |

### 4. 退化处理

如某步 Gate 失败（退化 >10%）：
1. 停止后续 apply
2. 检查该 milestone 的 commit 是否引入回归
...
- **禁止一次 apply**：即使看起来"所有 commit 都相关"，也必须拆分验证。一次性 apply 后不可定位退化 commit。
