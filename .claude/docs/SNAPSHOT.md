...
> 分支：uart-16550-lichee — Q17 QEMU 修复完成，Q19/Q19B 已完成并归档，Q19C-M0/M1/M2 已完成；M2 通过 `lichee-memory-root-command` 在 D1 真板完整运行 benchmark；M3 rootfs-probe 仍未通过，需先深度探索当前串口输出中断问题

---

## 当前状态

**分支**: uart-16550-lichee（Lichee RV Dock 适配与验证分支；Q17 QEMU 修复完成，Q19/Q19B 真板验证已完成，Q19C-M0/M1/M2 已完成；M3 rootfs-probe 真板输出不完整，进入深度探索；Q19D 已登记为 D1 SDMMC/rootfs 后续方向）
...
- **Q19C-M2 ✅ 已完成**: `lichee-memory-root-command` 在 D1 真板完整运行。`docs/M2.md` 记录 `shell_status=SKIPPED`、benchmark sections、`Done.`、`benchmark exited with code: 0` 和 `halting.`。true shell 不作为 Q19C 必达目标。
- **Q19C-M3 ⚠️ 未通过**: `docs/M3.md` 显示 rootfs-probe 启动并完成 async UART init，但只输出到 `d1_sdmmc_controller_base=TBD`，未打印完整 probe table、`rootfs_init=NOT called` 或 `probe complete`。当前不能把 M3 视为 board gate 通过。

**当前待推进**:
- **Q19C 🧪**: M0/M1/M2 已完成；M3 暂停直接实施，先做深度探索，范围包括 polling console 与 async UART 共享 UART0、entry cfg、输出 flush/中断路径和最小复现。
...
| Q19C-M0/M1/M2 数据边界 | 64B 小包旧异常主要是测量污染；D1 FIFO 16B burst 和 TTY short-write 修复已验证有收益；TX P99 长尾接受为 known limitation；M1 eager path 和 M2 command-entry 已通过，lazy file-backed COW 仍待单独修复。 |
| Q19C-M3 当前边界 | M3 不是 rootfs 成功，也不是完整 SDMMC probe 成功。当前只证明 rootfs-probe 入口和 async UART init 可到达；后续需先定位为什么 probe table 输出中断。 |
