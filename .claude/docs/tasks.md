...
| 分支 | 作用 | Milestone | 状态 |
|------|------|-----------|------|
| feat/uart-async | 主开发分支 | M0-M1 (基础设施 + 架构验证) | ✅ M1 完成 |
| feat/uart-async-m2 | **验证分支** — 内核内部测试代码 | M2 (VFS 验证) | ✅ 测试通过 |

**M2 验证分支说明**：
- **目的**: 添加内核内部测试代码，验证 M1 实现的功能正确性
- **添加文件**: `kernel/src/drivers/serial/test.rs` (M2 VFS 验证测试)
- **测试内容**: DeviceOps trait (write_at) + Pollable trait (poll) + TX 路径验证
- **验证方式**: 内核启动时自动执行测试，无需用户态程序
- **结果**: 所有自动化测试通过 ✅
- **后续**: 测试代码可保留为调试工具，或在正式版本移除
