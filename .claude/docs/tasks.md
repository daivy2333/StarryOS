...
> 最后更新: 2026-08-06 | 分支: net-k3 | grep: `<!-- T{编号} -->`
> 来源: R41、R47、R49、M41、D22、K31-K32、K37；NIC 开发已展开为 25 个单变量 milestones。MS01-MS03 与 MS16 已归档。
...
| <!-- T13 --> T13 | VF2 板级事实 Gate | 记录启动介质、DTS、DWMAC、PHY、MMIO、IRQ、DMA 和 hart 拓扑 | 每项来自真板、DTS 或手册；未知项阻塞后续设计 | T12；硬件可用 | ⏳ 等待 T12 |
| <!-- T14 --> T14 | VF2 启动与寄存器 | 接通 feature、镜像和 early console；只验证 DWMAC 寄存器访问 | 重复启动稳定；寄存器非全零/全一；异常访问可定位 | T13 | ⏳ 等待 T13 |
| <!-- T15 --> T15 | VF2 Clock/Reset/PHY | 按 D21 保留或恢复 clock/reset；只建立 PHY link | preserved 状态有原值；PHY ID、link 和协商结果可重复 | T14 | ⏳ 等待 T14 |
| <!-- T16 --> T16 | VF2 PLIC delivery | 只接 DWMAC IRQ claim、handler、status 和 EOI | IRQ claim 与设备 status 对齐；无中断风暴；per-hart 初始化可区分 | T15 | ⏳ 等待 T15 |
| <!-- T17 --> T17 | VF2 DMA/cache 基线 | 建立 DMA 地址转换、cache/barrier 和 descriptor ownership | CPU/设备看到同一 descriptor；ownership 转移有日志和断言 | T16 | ⏳ 等待 T16 |
| <!-- T18 --> T18 | DWMAC 轮询 RX | 只实现最小 RX descriptor refill/reap | 抓包与 descriptor 计数一致；坏帧和 ring full 有明确结果 | T17 | ⏳ 等待 T17 |
| <!-- T19 --> T19 | DWMAC 轮询 TX | 只实现最小 TX submit/reclaim | ARP/ICMP 发包可抓取；回收不重复；timeout 可诊断 | T18 | ⏳ 等待 T18 |
| <!-- T20 --> T20 | DWMAC 异步 RX | 将 T06 的 RX queue task 接到真板 IRQ | RX burst 无 lost wakeup；budget、drop 和 occupancy 可观测 | T19 | ⏳ 等待 T19 |
| <!-- T21 --> T21 | DWMAC 异步 TX | 将 T07 的 TX completion 和背压接到真板 | TX burst、queue full 和 flush 通过；无 descriptor 双重所有权 | T20 | ⏳ 等待 T20 |
...
### MS09：VF2 可观测平台与 PHY 基线

- Status: planned
- Outcome: VF2 可重复启动，DWMAC 寄存器和 PHY 状态可访问、可解释。
- Rationale: T13 是 T14-T15 的调查输入；三者共同形成可供驱动开发依赖的平台基线。
- Dependencies: MS01；VisionFive2 硬件可用
- Scope: T13-T15；启动介质、DTS、hart、feature、镜像、寄存器、clock/reset、MDIO 和 PHY。
- Non-goals: PLIC delivery、DMA descriptor、网络收发和异步队列。
- Workload: 板级事实、启动链、寄存器观测、U-Boot handoff 和 PHY link。
- Stable baseline: 重复启动稳定，寄存器非全零/全一，PHY ID、link 和协商结果可重复。
- Verification boundary: 每项事实来自真板、DTS 或手册，未知项明确阻塞。
- Diagnostic boundary: 失败限制在启动链、MMIO 映射、clock/reset 或 PHY。
...
- Status: planned
- Outcome: DWMAC 中断经 PLIC claim、handler、device status 和 EOI 可重复投递。
- Rationale: 真板 PLIC 与设备电平中断是独立高风险故障域，不能由 QEMU 证据替代。
- Dependencies: MS09
- Scope: T16；DWMAC IRQ 路由、primary/per-hart 初始化、cause、ack 和 EOI。
- Non-goals: DMA 收发、异步 queue task 和多 hart 流量。
- Workload: PLIC 路由、设备中断状态、重复触发和风暴诊断。
- Stable baseline: IRQ claim 与设备 status 对齐，EOI 后可再次触发。
- Verification boundary: 无中断风暴，primary/per-hart 初始化可区分。
- Diagnostic boundary: 失败限制在 VF2 PLIC、设备电平中断或 ack/EOI 顺序。
- Split signals: 单一 DWMAC 暴露多个必须独立验收的中断控制器路径。
- Related changes: None

### MS11：DWMAC 轮询双向网络基线

- Status: planned
- Outcome: 在明确 DMA/cache ownership 的前提下完成 DWMAC 轮询 RX/TX 和协议包收发。
- Rationale: T17 只有通过 T18-T19 的 descriptor 移动和真实包才能验证其 DMA/cache 契约。
- Dependencies: MS10
- Scope: T17-T19；DMA 地址转换、cache/barrier、descriptor ownership、最小 RX/TX 和抓包。
- Non-goals: 异步 wake、reset、SMP、offload 和零拷贝优化。
- Workload: DMA 抽象、descriptor ring、轮询收发、错误路径和协议验证。
- Stable baseline: CPU 与设备观察同一 descriptor，ARP/ICMP/UDP/TCP 与抓包一致。
- Verification boundary: RX/TX 回收不重复，坏帧、ring full 和 timeout 有明确结果。
- Diagnostic boundary: 失败限制在 DMA 地址、cache/barrier、descriptor 或轮询数据面。
...
- Status: planned
- Outcome: 已验证的 QEMU queue/stack 契约适配到 VF2 DWMAC 异步 RX/TX。
- Rationale: T20-T21 共享同一真板 IRQ、DMA 和队列适配边界，合并后形成双向 transport parity。
- Dependencies: MS06, MS11
- Scope: T20-T21；DWMAC RX/TX completion、budget、slot、backpressure 和 flush。
...
- QEMU 证据不替代 VF2 DMA、cache、PHY、SMP 或性能证据。
