...
| <!-- L219 --> | 32-bit MMIO 缺口 | `uart_16550` MMIO backend 有 stride 但 volatile access 是 `u8`；D1/VF2 DW APB UART 需要显式 access width，不能只把 stride 改成 4。 | R6 |
| <!-- L220 --> | early console 分层 | 真板 bring-up 先用不依赖 IRQ、async task、rootfs 的 polling early console；descriptor 稳定后再接 async UART `/dev/console`。 | R6 |
...
| <!-- L240 --> | Q19B 三模式拆分 | `lichee-d1` smoke、`lichee-d1-kbench` 内核 benchmark、`lichee-d1-userbench` 用户态 benchmark 分离；kbench 启用真 PLIC，公共初始化走 `lichee_d1_init()`。 | Q19B Phase 1 |
| <!-- L241 --> | D1 UartPort | `kernel/src/drivers/d1_uart.rs` 实现 `ArceOsD1UartPort`：stride-aware `u32` volatile MMIO，RBR/THR=0、IER=1、IIR=2、LSR=5；D1 ISR 复用 RX/TX/DRAIN waker。 | ADR-048 |
| <!-- L242 --> | QEMU/D1 双路径 gate | `uart_init.rs` 用 cfg 分离 QEMU byte-MMIO 与 D1 32-bit MMIO；common 部分保留 ring、driver、ISR 注册、copier 启动。 | Q19B Phase 2 |
...
| <!-- L248 --> | Q19B feature 规范 | `lichee-d1-async-uart` 表示硬件能力；`lichee-d1-kbench` / `lichee-d1-userbench` 表示运行模式。kbench 排除项不能影响 userbench；D1 排除 net/fb/display/PCI 假设。 | ADR-050 |
| <!-- L249 --> | D1 userbench 最小依赖 | 需要 `dep:axfs` + `axfeat/task-ext`；不需要 display、virtio block、net、PCI 常量。socket/net/fb/display 路径在 D1 下 cfg 排除。 | ADR-049 |
| <!-- L250 --> | D1 embedded benchmark | `kernel/resources/benchmark.elf` 用 `include_bytes!` 嵌入；`load_embedded_user_app()` 复用 ELF/AUXV/heap/stdio 初始化。payload 必须是 `ET_EXEC`、no relocation、no static PIE。 | Q19B-Next.4 |
| <!-- L251 --> | Q19B host gate | `lichee-d1`、`lichee-d1-kbench`、`lichee-d1-userbench`、`qemu` 四模式 cargo check 通过；`make lichee-kbench` 生成 188608B kernel，`make lichee-userbench` 生成 876736B kernel。 | Q19B 收尾 |
| <!-- L252 --> | axdriver cfg(bus) | `axdriver/build.rs` 无 `bus-mmio` 时默认 `cfg(bus="pci")`；定位看 `target/*/build/axdriver-*/output`。 | axfs-ng patch |
| <!-- L253 --> | axfeat 弱转发边界 | `axfeat/bus-mmio` 不能影响 `axfs-ng -> axdriver` 间接依赖；本地 `crates/axfs-ng/Cargo.toml` 必须显式设置 `axdriver default-features=false, features=["block","bus-mmio"]`。 | axfs-ng patch |
| <!-- L254 --> | embedded ELF 禁 PIE | D1 embedded benchmark 必须用 `-static -no-pie -fno-pie -s`；用 `file`、`readelf -h`、`readelf -r` 验证 `ET_EXEC` 且无 relocation。 | Q19B |
| <!-- L255 --> | D1 THRE/no-pending | D1 IRQ 18 可进但 IIR 常为 `0xc1` no pending，有效 THRE `0xc2` 偶发；启用 THRE 时若 LSR 已 ready，必须软件 wake TX/DRAIN。 | ADR-051 |
| <!-- L256 --> | tcdrain/drain wake | `tcdrain` 卡因等待者只看 TX ring，未覆盖 staged/TEMT。`flush()` / `TCSBRK` 注册 `DRAIN_WAKER`，TX copier 最后一批 TEMT 后 wake drain。 | ADR-051 |
| <!-- L257 --> | D1 async UART 基线 | Q19B: 256B 11.25KB/s、1024B 11.40KB/s、4096B 11.41KB/s；1B avg 0.270ms、P50 0.185ms、P99 8.547ms；FIONBIO 双入口 PASS。64B 旧 1.01KB/s 被 Q19C-M0 纠正，见 L265。 | Q19B |
| <!-- L258 --> | 串口输出清理 | 真板终端需 CRLF；stdout 退出前需 `fflush(stdout); tcdrain(STDOUT_FILENO);`。修改 C 后需重编 `benchmark.elf`，当前沙箱运行 musl gcc 会 `Bad system call`。 | Q19B |
| <!-- L259 --> | Q19C path 差异 | QEMU 走 rootfs `/bin/benchmark -> load_user_app -> /dev/console`；D1 Q19B 走 embedded ELF。Q19C-M1 证明 D1 memory-root `/bin/benchmark` 可经 `resolve/read` + eager mapping 完整运行；lazy COW SIGILL 单列 L277。 | R10 |
| <!-- L260 --> | D1 fullbench feature | Q19C 不启用 `qemu` feature；`lichee-d1-fullbench` / command mode 继承 D1 async UART/PLIC、paging、task-ext、axfs，排除 QEMU PCI/virtio/display/rootfs 假设。 | ADR-052 |
| <!-- L261 --> | D1 rootfs 边界 | populated memory root 足够证明 VFS path loader；真实 SDMMC/block/rootfs 不再属于 async UART 性能验证，需要时重新 propose。 | ADR-052 |
| <!-- L262 --> | benchmark 证据口径 | manifest 必须记录 version、mode/startup chain、root provider、payload size、iters、drain policy、timer、nonblocking entries、RX mode。QEMU、embedded、memory-root path、command 分组解释。 | Q19C |
| <!-- L263 --> | Q17 D1 边界 | D1 `update_ier()` 也有 RMW 形态，但 D1/C906 是单核；O63 结论仍以 QEMU/VF2 多 hart 为准。实施时同步收敛 QEMU/D1 `UartPort::update_ier()` 契约，或注明 D1 暂非 SMP 风险。 | R3 |
| <!-- L264 --> | Q17 收尾边界 | QEMU/D1 已完成单 hart/单板回归：QEMU 64B TX 153.86KB/s、1B avg 0.182ms、FIONBIO PASS；`tx_copier_active` Release/Acquire，`tx_staged_bytes` AcqRel/Acquire。multi-hart 仍需 Q24 stress。 | Q17 |
| <!-- L265 --> | Q19C 64B 测量边界 | 旧 64B 约 1KB/s 来自 stdout backlog 污染（pre-section drain 5688ms）。每节开始必须 `fflush+tcdrain` 并打印 pre-drain；隔离后 D1 64B 为 10.7-11.1KB/s（93-97% 线速）。绝对 `slow_gt10ms` 不适合大包，应用相对线时指标。 | Q19C-M0 |
| <!-- L266 --> | D1 TX drain/THRE 长尾 | D1 THRE 表示 FIFO 可重新填充，不应每字节重读；一次 THRE 后最多填 16B。`TX_FAST_RETRY_LIMIT=0` + drain `TX_WAKER` 会卡，当前保留 32 次 fast retry + `DRAIN_WAKER`。 | Q19C |
| <!-- L275 --> | slow-poll/yield 结果 | 三轮真板：slow-poll 让 `hw_send_zero` 涨 20x 但 P99 不变；yield 重试未触发；`slow_poll_exh=0`，说明 forward progress 未丢。P99 根因未明，影响 <2%，Q20 复验。 | Q19C.8e |
| <!-- L276 --> | M1 memory-root API | `init_memory_root()` 后用 `FS_CONTEXT.create_dir/write("/bin/benchmark")` 注入，再 `resolve/read`；真板通过路线是 `load_user_app_eager_from_path()`。不新增 tmpfs 内部写接口，不把 fullbench 继续走 embedded loader。 | R12 |
| <!-- L277 --> | lazy file-backed COW SIGILL | `load_user_app()` lazy tmpfs/COW 可进用户态并处理 page fault/syscall，但 main 前在合法 RV64C `c.ld` (`ip=0x151d4`) SIGILL。更可能是 `CachedFile/FileBackend::Cached + Backend::new_cow` 取指/读字节问题，不是 UART。 | O80 |
| <!-- L278 --> | M2 command-entry | 仓库无已知静态 `/bin/sh`；`.sh` 会转 `/bin/sh` 且有 `/proc/self/exe` FIXME。M2 用 equivalent command entry，记录 label、argv/envp、stdio、spawn/join、exit code；不能伪称 shell success。 | R13 |
| <!-- L279 --> | M3/rootfs 边界 | D1 无注册到 `AxBlockDevice` 的 SDMMC driver；`axfs-ng::init_filesystems()` 空 block list 会 panic。storage/rootfs 不是 async UART gate，需重新 propose。 | R13 |
| <!-- L280 --> | M3 startup 风险 | M3 日志只到 `[BENCH] FIFO depth: 16 bytes`，未进 rootfs-probe 分支；只影响 probe evidence，不影响 D1 async UART 结论。除非重开 storage/rootfs，不再隔离。 | Q19C closeout |
| <!-- L281 --> | 真板验证分层 | VisionFive2/DWMAC 经验：先验证 FIT/串口/MMIO 可访问，再接 DMA/IRQ/workload。StarryOS Q24 按 boot、寄存器、PLIC、waker、drain、stress、userbench 分层。 | R14 |
| <!-- L282 --> | VF2 hart 拓扑 | VF2 Boot HART ID=1、HART Count=5；CPU0 是 S7 小核，不适合 `riscv64gc` 路径。Q24 不能假设 hart0 是主核或可跑同一 ISA。 | R14 |
