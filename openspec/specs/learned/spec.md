...
| <!-- L275 --> | Q19C.8e slow-poll + yield 重试真板验证结果 | 真板三轮数据对比：(1) 基线 `TX_FAST_RETRY_LIMIT=32` 无 slow-pool → P99=50.872ms；(2) 加 slow-pool（`TX_SLOW_POLL_LIMIT=4096` × `TX_SLOW_POLL_SPINS=256`）→ `hw_send_zero` 涨 20x 但 `no_progress_budget` 不变，P99 不变；(3) 加 yield 重试（`TX_YIELD_RETRIES=4` 自唤醒）→ `slow_poll_exh=0` `yield_exh=0`，P99 不变。结论：slow-pool 100% 成功（每次 budget exhausted 后约 653 次 send_bytes 后 FIFO 排空），P99 长尾**根因未探明**——`slow_over_line_plus10ms=1`（100 次中 1 次超出线时+10ms），slow-pool/yield 均未改善，推测非 ISR 丢失（`slow_poll_exh=0`）但确切成因待查。当前影响可接受（吞吐量 <2%），暂不继续优化，Q20 复验时再探明。`hw_send_zero` 高是 slow-pool 的预期副产物（653 次返回 0 后 1 次成功），不影响吞吐量（line_rate 96.6-98.8%）。Q19B regression 全通过：exit code 0、`hw_send_max_chunk=16`、S30 FIONBIO PASS。 | Q19C.8e 真板验证 2026-07-07 |
