...
| <!-- L243 --> | Q19B Phases 5-6 被 axfs 阻塞 | `lichee-d1-userbench` 的 Phase 5 (`/dev/console` TTY gate) 需要 `pseudofs::mount_all()` → `axfs::{FS_CONTEXT, FsContext}` 和 `axfs_ng_vfs`，这些 crate 当前仅在 QEMU feature set 中可用。引入 `axfs` 会连带引入 block device/virtio/PCI 假设，与 D1 硬件冲突。且 `pseudofs::mount_all()` 内部还会挂载 `/dev/shm`、`/tmp`、`/proc`、`/sys` 等路径。解决方案需要独立立项：要么为 D1 实现最小化 devfs（不依赖 axfs），要么将 axfs 适配到 D1 平台。当前 `kernel/src/lib.rs` 已将 `file`/`pseudofs`/`mm`/`syscall`/`task`/`time` 模块从 kbench 路径排除（`#[cfg(not(any(feature = "lichee-d1-smoke", feature = "lichee-d1-kbench")))]`）。详见 ADR-049。 | Q19B Phase 5-6 阻塞 |
...
| <!-- L246 --> | Q19B 五个阻塞点全景 | (1) userbench 继承 kbench 导致模块排除；(2) `/dev/console` 需要 TTY/devfs/syscall 全路径而非仅 UART；(3) `pseudofs::mount_all()` 假设 `axfs` 存在但 D1 不启用 qemu feature；(4) embedded benchmark ELF 不是当前第一优先级——先让 userbench 编译通过并 bring up `/dev/console`；(5) 真板证据缺失：PLIC IRQ 18 / RX-TX wake path / ring-buffer 指标 / tcdrain 行为均待真板日志。结论：Q19B 不再被 D1 UART MMIO 模型阻塞，而是被 kernel benchmark image 与 user benchmark image 之间的运行时边界阻塞。 | `.claude/analysis/q19b-current-blockers.md` §Blockers |
| <!-- L247 --> | Q19B-Next plan 五步推进路线 | Q19B-Next.1: 规范化 feature 语义（hw capability ≠ runtime mode）；Next.2: `cargo check lichee-d1-userbench` host gate 通过；Next.3: `/dev/console` 先于 ELF payload bring up；Next.4: 嵌入 `tests/benchmark.c` 静态 RISC-V ELF；Next.5: 采集真板串口日志到 `.claude/analysis/lichee/q19b-*.txt`。禁止在 `/dev/console` gate 通过前加载完整 benchmark；禁止通过启用 QEMU PCI/virtio feature 绕过。 | `.claude/analysis/q19b-current-blockers.md` §Recommended Next Plan |
