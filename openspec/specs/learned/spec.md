...
| <!-- L211 --> | Q15 后 milestone 重排启发式 | 不按 O 编号顺序排期，按 Gate 类型分层：文档/规格收敛 → QEMU 可验证 correctness → 真板观测脚手架 → 真板 bring-up → 数据驱动决策 → 维护性清理 → 远期实验。避免把 O63/O64/O66/O3/O40/O41/O48 等不同触发条件的项继续塞进单一 Q6 | `.claude/analysis/optimization-milestone-replan.md` `[ARCHIVED 2026-07-04 → _archive/2026-07-04-q19-lichee-analysis/optimization-milestone-replan.md]` |
...
| <!-- L215 --> | Lichee RV Dock boot image 事实 | `/dev/mmcblk0p4` / `boot` header magic `ANDROID!`，name `d1-nezha`，page_size `2048`，kernel_size `9783580`，kernel_addr `0x40200000`，ramdisk_addr `0x41200000`，tags_addr `0x40200100`。StarryOS smoke test 优先生成 Android boot image 写入 boot 分区；写入前必须备份原 boot。 | `tests/boot_probe.c` 输出 `.claude/analysis/lichee/probe` `[ARCHIVED 2026-07-04 → _archive/2026-07-04-q19-lichee-analysis/lichee/probe]` |
| <!-- L216 --> | Lichee RV Dock early smoke test 路线 | 下一步工程目标不是继续采集，而是新增 D1 platform + DW APB UART early console + Android boot image 打包。成功标准：串口输出 `[starry-d1] early boot`。初期 timer 优先走 SBI timer，rootfs/USB/SD/Shell/async benchmark 后置。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/lichee/licheerv-dock-bringup.md` |
| <!-- L217 --> | 平台参数耦合点速查 | `kernel/src/drivers/uart_init.rs` 当前硬编码 QEMU UART：base `0x10000000`、stride 1、raw LSR `base+5`、`iomap(..., 0x1000)`；换 Lichee / VisionFive2 时不能只改构建目标，必须把 UART facts 从 driver init 中抽出。 | `.claude/analysis/platform-parameter-decoupling.md` `[ARCHIVED 2026-07-04 → _archive/2026-07-04-q19-lichee-analysis/platform-parameter-decoupling.md]` |
...
| <!-- L259 --> | Q19C 完整 benchmark 路径差异 | QEMU 完整路径通过 `entry::init -> pseudofs::mount_all -> FS_CONTEXT.resolve(args[0]) -> load_user_app -> add_stdio(/dev/console)` 启动；D1 Q19B userbench 当前通过 `init_memory_root -> include_bytes!(benchmark.elf) -> load_embedded_user_app` 绕过 rootfs/path loading。Q19C-M1 已先让 D1 从 memory root 的 `/bin/benchmark` 走 `FS_CONTEXT.resolve()/read()` + eager ELF segment mapping，证明路径可见文件、用户进程、stdio 和 benchmark 正常；`load_user_app()` lazy file-backed COW 在 memory-root/tmpfs 上的 SIGILL 另行处理。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/q19c-lichee-full-starryos-benchmark.md`, `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/lichee/Q19cM1.md` |
| <!-- L260 --> | D1 fullbench feature 边界 | Q19C 不应启用 `qemu` feature；独立 `lichee-d1-fullbench` / command mode 继承 D1 async UART/PLIC、paging、task-ext、axfs，但继续排除 QEMU PCI/virtio/display/rootfs 假设。2026-07-11 方向更新后，shell、SDMMC、block 和真实 rootfs 不再是 Q19C gate。 | Q19C 探究 / 2026-07-11 方向更新 |
| <!-- L261 --> | D1 rootfs provider 分层 | 历史分层是先用 populated memory root 提供 `/bin/benchmark` 证明 VFS path loader，再考虑真实 SDMMC/block rootfs。2026-07-11 后，真实 rootfs/provider 不再属于 async UART 性能验证目标；需要 storage/rootfs bring-up 时必须重新 propose。 | Q19C 探究 / 2026-07-11 方向更新 |
| <!-- L262 --> | Q19C benchmark 证据口径 | Q19C benchmark manifest 必须记录 benchmark version、mode/startup chain、root provider、payload sizes、iteration counts、drain policy、timer source、nonblocking entries、RX test mode。QEMU、Q19B embedded、memory-root path、memory-root command 的参数不同就分组解释，不能混成一条横向性能曲线。Shell/rootfs 不再作为当前对比维度。 | Q19C 重新探索 / 2026-07-11 方向更新 |
...
| <!-- L264 --> | Q17 收尾验证边界 | Q17 已完成 QEMU 单 hart修复与回归：QEMU `update_ier()` cache RMW 与 MMIO IER 写同锁，D1 `update_ier()` 用 IRQ-off 临界区，`tx_copier_active` 为 Release/Acquire，`tx_staged_bytes` 为 AcqRel/Acquire；`make run` 默认 `BUS=mmio` 后 rootfs `/bin/benchmark` 通过（64B TX 153.86 KB/s、1B latency avg 0.182 ms、FIONBIO PASS）。这不等于多 hart 内存序已实测，Q20/VisionFive2 或等价 SMP stress 仍必须复验。 | Q17 2026-07-03 收尾 |
| <!-- L265 --> | Q19C 64B 小包性能测量边界 | 2026-07-07 新数据表明，旧 64B `write+tcdrain` 约 1KB/s 异常主要来自测量边界污染：进入 S10 前 stdout backlog 在 D1 上 `pre_section_stdout_drain_ms=5688`，会把 manifest/日志排空时间计入首个小包段。每节开始必须 `fflush(stdout); tcdrain(STDOUT_FILENO)` 并打印 pre-drain；隔离后 D1 64B 为 10.7-11.1KB/s（约 93-97% 线速）。`slow_gt10ms` 这类绝对阈值不适合 128B/256B/1024B，应改用相对物理线时的超时指标。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/lichee/NewDate.md` Q19C-M0 |
| <!-- L266 --> | Q19C TX drain/THRE 长尾排查经验 | D1 `LSR_THRE` 应按“TX FIFO 可重新填充/近似空”处理，不应每写 1B 都重新读 THRE；`ArceOsD1UartPort::send_bytes()` 在 THRE 后一次填充最多 16B，这一项已在真板数据中验证能把 `hw_send_max_chunk` 提到 16 并恢复 1024B 正确发送。`TX_FAST_RETRY_LIMIT=0` 加 `tcdrain/flush` 注册 `TX_WAKER` 的尝试会在 benchmark 进程启动后卡住，说明 D1 THRE 唤醒/补唤醒路径还不能作为唯一进展来源；后续要先保持已验证的 32 次 fast retry + `DRAIN_WAKER` drain 路径，再单独设计低风险机制压低 zero-send 与 P99 长尾。 | Q19C D1 TX copier 修复回滚 |
| <!-- L275 --> | Q19C.8e slow-poll + yield 重试真板验证结果 | 真板三轮数据对比：(1) 基线 `TX_FAST_RETRY_LIMIT=32` 无 slow-pool → P99=50.860ms；(2) 加 slow-pool（`TX_SLOW_POLL_LIMIT=4096` × `TX_SLOW_POLL_SPINS=256`）→ `hw_send_zero` 涨 20x 但 `no_progress_budget` 不变，P99 不变；(3) 加 yield 重试（`TX_YIELD_RETRIES=4` 自唤醒）→ `slow_poll_exh=0` `yield_exh=0`，P99 不变。结论：slow-pool 100% 成功（每次 budget exhausted 后约 653 次 send_bytes 后 FIFO 排空），P99 长尾**根因未探明**——`slow_over_line_plus10ms=1`（100 次中 1 次超出线时+10ms），slow-pool/yield 均未改善，推测非 ISR 丢失（`slow_poll_exh=0`）但确切成因待查。当前影响可接受（吞吐量 <2%），暂不继续优化，Q20 复验时再探明。`hw_send_zero` 高是 slow-pool 的预期副产物（653 次返回 0 后 1 次成功），不影响吞吐量（line_rate 96.6-98.8%）。Q19B regression 全通过：exit code 0、`hw_send_max_chunk=16`、S30 FIONBIO PASS。 | Q19C.8e 真板验证 2026-07-07 |
| <!-- L276 --> | Q19C-M1 memory-root path loader API 速查 | `pseudofs::init_memory_root()` 只建立空 tmpfs root；M1 应用 `FS_CONTEXT.lock().create_dir("/bin", 0o755)` + `FS_CONTEXT.lock().write("/bin/benchmark", include_bytes!("../resources/benchmark.elf"))` 注入 benchmark，再 `resolve("/bin/benchmark")` 验证路径可见。真板通过路线是 `load_user_app_eager_from_path()`：`FS_CONTEXT.read("/bin/benchmark")` 后复用 eager ELF segment mapping；不要新增 tmpfs 内部写接口，也不要把 fullbench 成功路径继续走 `load_embedded_user_app()`。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/q19c-m1-memory-root-path-loader.md`, `kernel/src/mm/loader.rs` |
| <!-- L277 --> | Q19C-M1 lazy file-backed loader 踩坑 | 直接走 `load_user_app()` 的 memory-root/tmpfs lazy file-backed COW 路径可进入 benchmark 进程、处理若干 EXEC/READ page fault，并完成 `set_tid_address` syscall，但在 main 前 SIGILL 于 `ip=0x151d4`；该地址反汇编为合法 RV64C `c.ld a5,32(a0)`，因此不是编译器产出非法指令，也不是 async UART 问题，更可能是 `CachedFile/FileBackend::Cached + Backend::new_cow` 在 tmpfs/memory-root 上的懒映射取指/读字节问题。收敛方法：先用少量 page fault/syscall/signal 诊断定位阶段，确认后清理诊断日志；M1 用 VFS read + eager mapping 通过，lazy COW 修复作为独立 loader/mm 后续项。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/lichee/Q19cM1.md`, Q19C-M1 真板调试 2026-07-08 |
| <!-- L278 --> | Q19C-M2 command-entry 边界 | 当前仓库没有已知可嵌入的静态 `/bin/sh`；`load_user_app()` 对 `.sh` 会重定向到 `/bin/sh`，且源码已有 `/proc/self/exe` busybox FIXME。M2 记录 `shell_status=SKIPPED` 并走 documented equivalent command entry：独立 label、明确 argv/envp、stdio `/dev/console`、spawn/join 和 exit code。2026-07-11 后，M2 command-entry 是 Q19C 收尾 gate，不能伪称 shell success。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/q19c-m2-m3-shell-sdmmc-probe.md`, `kernel/src/mm/loader.rs`, `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/lichee/M2.md` |
| <!-- L279 --> | Q19C-M3 SDMMC/rootfs probe-only 边界 | D1 当前没有注册到 `axdriver::AxBlockDevice` 的 SDMMC driver；`axfs-ng::init_filesystems()` 会对空 block list `expect("No block device found!")`。该事实说明 storage/rootfs 不是 async UART 性能验证条件。2026-07-11 后，M3/rootfs-probe 取消为当前 gate；需要 storage/rootfs 时重新 propose。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/q19c-m2-m3-shell-sdmmc-probe.md`, `crates/axfs-ng/src/lib.rs` |
| <!-- L280 --> | Q19C-M3 startup benchmark 前置风险 | M3 真板日志只到 `[BENCH] FIFO depth: 16 bytes`，未进入 `kernel/src/entry.rs` 的 `lichee-d1-rootfs-probe` 分支。该问题只影响 rootfs-probe evidence，不影响已完成的 D1 async UART 性能结论。2026-07-11 后，不再继续隔离 M3 entry，除非重新打开 storage/rootfs bring-up。 | `.claude/analysis/_archive/2026-07-11-q19c-d1-async-uart-closeout/`, `kernel/src/entry.rs`, `kernel/src/drivers/bench.rs` |
| <!-- L281 --> | ArceOS 真板验证分层方法 | 明扬在 VisionFive2/DWMAC 上先验证 FIT image 启动链、串口日志、MMIO 寄存器可访问性，再接 DMA ring、中断、ping/HTTP workload；不把“完整 OS 一次跑通”作为第一层 gate。StarryOS Q20 应按 boot/寄存器/PLIC/waker/drain/stress/userbench 分层推进。 | `.claude/analysis/arceos-true-board-validation.md`, `work/arceos/scripts/make/starfive.mk`, `work/arceos/modules/axasync/LOG.md` |
| <!-- L282 --> | VisionFive2 多 hart 复验前先确认 hart 拓扑 | arceos 真板日志显示 Boot HART ID=1、HART Count=5；CPU0 是 S7 小核，不适合 `riscv64gc` 路径，明扬在 `modules/axruntime/src/mp.rs` 跳过 CPU0 只启动 U74 核。Q17/Q20 不能默认 hart0 是主核或可跑同一 ISA。 | `.claude/analysis/arceos-true-board-validation.md`, `work/arceos/docs/visionfive2_heterogeneous_cpu_fix.md`, `work/arceos/modules/axruntime/src/mp.rs` |
