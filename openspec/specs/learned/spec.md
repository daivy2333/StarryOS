...
| <!-- L276 --> | Q19C-M1 memory-root path loader API 速查 | `pseudofs::init_memory_root()` 只建立空 tmpfs root；M1 应用 `FS_CONTEXT.lock().create_dir("/bin", 0o755)` + `FS_CONTEXT.lock().write("/bin/benchmark", include_bytes!("../resources/benchmark.elf"))` 注入 benchmark，再 `resolve("/bin/benchmark")` 验证路径可见。真板通过路线是 `load_user_app_eager_from_path()`：`FS_CONTEXT.read("/bin/benchmark")` 后复用 eager ELF segment mapping；不要新增 tmpfs 内部写接口，也不要把 fullbench 成功路径继续走 `load_embedded_user_app()`。 | `.claude/analysis/q19c-m1-memory-root-path-loader.md`, `kernel/src/mm/loader.rs` |
| <!-- L277 --> | Q19C-M1 lazy file-backed loader 踩坑 | 直接走 `load_user_app()` 的 memory-root/tmpfs lazy file-backed COW 路径可进入 benchmark 进程、处理若干 EXEC/READ page fault，并完成 `set_tid_address` syscall，但在 main 前 SIGILL 于 `ip=0x151d4`；该地址反汇编为合法 RV64C `c.ld a5,32(a0)`，因此不是编译器产出非法指令，也不是 async UART 问题，更可能是 `CachedFile/FileBackend::Cached + Backend::new_cow` 在 tmpfs/memory-root 上的懒映射取指/读字节问题。收敛方法：先用少量 page fault/syscall/signal 诊断定位阶段，确认后清理诊断日志；M1 用 VFS read + eager mapping 通过，lazy COW 修复作为独立 loader/mm 后续项。 | `docs/Q19cM1.md`, Q19C-M1 真板调试 2026-07-08 |
| <!-- L278 --> | Q19C-M2 shell/script 等价入口边界 | 当前仓库没有已知可嵌入的静态 `/bin/sh`；`load_user_app()` 对 `.sh` 会重定向到 `/bin/sh`，且源码已有 `/proc/self/exe` busybox FIXME。M2 若无 shell，应记录 `SKIPPED` 并走 documented equivalent command entry：独立 label、明确 argv/envp、stdio `/dev/console`、spawn/join 和 exit code，不能把 kernel-launched benchmark 伪称为 shell success。 | `.claude/analysis/q19c-m2-m3-shell-sdmmc-probe.md`, `kernel/src/mm/loader.rs` |
| <!-- L279 --> | Q19C-M3 SDMMC/rootfs probe-only 边界 | D1 当前没有注册到 `axdriver::AxBlockDevice` 的 SDMMC driver；`axfs-ng::init_filesystems()` 会对空 block list `expect("No block device found!")`。M3 应先输出 `lichee-rootfs-probe` 表和 `SKIPPED: missing D1 SDMMC/block driver`，只在 registered block device 存在后才调用 rootfs 初始化。`simple-sdmmc` 可作为 PIO-first 参考，但不是现成 D1 block provider。 | `.claude/analysis/q19c-m2-m3-shell-sdmmc-probe.md`, `crates/axfs-ng/src/lib.rs` |
| <!-- L280 --> | Q19C-M3 startup benchmark 前置风险 | `docs/M3.md` 真板日志只到 `[BENCH] FIFO depth: 16 bytes`，未进入 `kernel/src/entry.rs` 的 `lichee-d1-rootfs-probe` 分支。原因边界：M3 在 rootfs-probe 证据前无条件执行 `bench::run_startup_benchmark()`；该 benchmark 属于 D1 async UART/TX ring/调度刷出路径，不是 SDMMC/rootfs probe 证据。M3 后续应隔离入口：probe-only 模式先输出 blocker table 和 halt；startup benchmark 稳定性另列优化项处理。 | `docs/M3.md`, `kernel/src/entry.rs`, `kernel/src/drivers/bench.rs` |
