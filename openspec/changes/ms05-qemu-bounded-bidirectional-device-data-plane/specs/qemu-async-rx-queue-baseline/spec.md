...
#### Scenario: VirtIO-MMIO 适配双向 queue contract
...
#### Scenario: 使用 DWMAC 模型审查双向 contract
...
#### Scenario: 双向 queue service 成功激活
...
#### Scenario: Used-ring IRQ 唤醒双向 queue service
...
#### Scenario: RX 或 TX event-before-register
...
#### Scenario: Queue service 开始有界双向 drain
...
#### Scenario: 依赖不暴露有效双向 EVENT_IDX 控制
...
#### Scenario: 任一阶段 budget exhausted
...
#### Scenario: Spurious 或 software nudge 无数据工作

- **WHEN** task 因 spurious IRQ 或 software nudge 醒来且两个 used ring 与 slots 均无可推进工作
...
#### Scenario: RX completion 进入最终 slot
...
#### Scenario: Caller-driven Service 消费 RX slot
...
#### Scenario: MS05 runtime Evidence 完整
