...
change-local probe使用`poll(NULL, 0, timeout)`执行有界采样等待。RISC-V musl把该调用转换为`ppoll` syscall 73；`nfds==0`时kernel必须忽略`fds`，由既有空`FdPollSet`与timer future等待并返回0。修复只在poll syscall边界生成安全空slice，并同时覆盖x86_64 `sys_poll`的同构语义；不得让通用`UserPtr::get_as_mut_slice(0)`从NULL构造Rust slice，也不得用nanosleep或probe特判绕过产品缺陷。`nfds>0`时NULL或不可访问`fds`仍返回`EFAULT`。
...
- [零`nfds`修复误扩张通用用户内存语义] → 只在`sys_poll`/`sys_ppoll`参数归一化处返回安全空slice；正`nfds`与其他`UserPtr`调用者保持现有校验。
