...
- 决策：Q31 只做 baseline 快照、timer 修正、benchmark 实现和 Async QEMU/D1 证据；
  `q32-console-cpu-efficiency-benchmark` 复用同版合同，独立完成 Console 与 comparison。
- 原因：用户于 2026-07-22 批准不再向 Q31 增加 Console iteration；独立 change 能隔离
  分支代码、证据目录和归档生命周期。
- 影响：Q31 evidence 成为 Q32 按 hash 引用的只读输入，Console 任务不再由 Q31 Act 执行。
- 替代：在 Q31 创建后续 Console iteration。拒绝，因为会继续混合两个分支生命周期。
...
| R8 baseline and cross-branch evidence | Q31 1.1, 5.1, 5.3-6.5, 7.1-7.10；Q32 1.1-1.4, 5.1-7.5 | 100% | Q31 提供冻结 Async 输入；Console 与 comparison 由用户批准的 Q32 独立完成 | Covered across changes |
| R9 existing UART behavior unchanged | 1.4, 2.4, 3.2, 4.6, 5.2, 6.3, 7.1-7.6, 8.1-8.3, 9.1-9.5 | 100% | None | Covered |

Console 工作转移到独立 Q32，不是需求裁剪。Q31 的 Async 证据与 Q32 的 Console/comparison
任务共同覆盖跨分支要求。
...
`README.md` 记录 Async 文件的 source branch、commit、benchmark hash、build/run command、
target、hart 数、串口配置和 SHA-256。Console 与 comparison 位于
`.claude/analysis/q32-console-cpu-efficiency-evidence/`，并引用本目录的固定 hash。
...
5. 由独立 Q32 change 在 `console-lichee` 复用相同 benchmark 版本和计时修正。
6. Q32 保存 Console 新日志、引用 Q31 hash 并生成 comparison。
