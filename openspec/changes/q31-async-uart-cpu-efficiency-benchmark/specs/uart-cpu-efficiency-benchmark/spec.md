...
change MUST 将可覆盖的 `docs/*out.md` 与不可变 evidence 分离，保存 Async 同口径原始日志
和来源信息，并为独立 Console change 提供可按 hash 引用的冻结输入。
...
- **WHEN** Q31 Async 与独立 Q32 Console 的新 D1 日志都已采集
...
- **AND** Q32 MUST 以 hash 引用 Q31 输入且 MUST NOT 改写已验收的 Q31 Async evidence
