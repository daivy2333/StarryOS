...
- [x] 7.2 iteration 003 Review 通过后按用户批准创建独立 `q32-console-cpu-efficiency-benchmark`，不在 Q31 新增 Console iteration。[R8,R9]
...
- [x] 7.6 iteration 003 通过 Plan Review 后勾选 7.2，并把 Console/comparison 所有权转移到 Q32；Async 诊断字段不再触发重测。[R1-R9]
...
## 8. Console Follow-up Transfer

- [x] 8.1 将 benchmark 与 D1 time conversion 的 Console 移植责任转移到 Q32 tasks 2.1-4.7；Q31 不再修改 `console-lichee` 产品文件。[R1-R9]
- [x] 8.2 将 Console QEMU 采集责任转移到 Q32 tasks 5.1-5.4，证据写入 Q32 独立目录。[R2-R8]
- [x] 8.3 将 Console D1 采集责任转移到 Q32 tasks 6.1-6.5，Q31 Async 日志保持只读。[R1-R9]
- [x] 8.4 将 Console provenance 与可覆盖 docs 的处理转移到 Q32 tasks 1.1-1.4、7.1；Q31 baseline SHA-256 不变。[R8]

## 9. Comparison and Change Gate Transfer

- [x] 9.1 将 Async/Console common-field comparison 转移到 Q32 tasks 7.1-7.5；Q31 只提供冻结 Async raw 输入。[R2-R9]
- [x] 9.2 将中立结论与“等待转移到后台”的解读约束转移到 Q32 requirement Q32-R10，不设性能胜负阈值。[R3-R5,R9]
- [x] 9.3 iteration 003 已运行 Q31 strict、全局 changes/specs validate 与 `git diff --check` 并记录 PASS；Q32 另行执行自己的 Gate。[R1-R9]
- [x] 9.4 iteration 003 已确认 Q31 收口未修改 UART copier、THRE retry、IER、waker、TTY、drain 或 debug ABI 语义。[R9]
- [x] 9.5 Q31 不因 Q32 的独立生命周期新增 iteration、更新全局状态或自动归档；后续生命周期操作由用户另行批准。[R8,R9]
